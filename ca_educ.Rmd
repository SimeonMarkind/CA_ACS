...
California seems to be a well educated state, with roughly 40% of the population over 25 holding an Associate's degree or higher. Interestingly this percentage seems relatively flat for those under age 65, despite the growing segment of the US population attending college. This could be due to people starting college and not completing their degrees, resulting in the population segment with a degree remaining constant despite enrollment increasing overall. Luckily it is possible for us to test this as the ACS breakdown of education includes the option of having attended college but not completing a degree.

```{r "Population with some college", results="show"}
educ.data[SCH == 1, .(age = round_any(AGEP, 3), PWGTP, SCHL)][,
...
As we can see from the above chart, about 20% of the adult population in California above the age of 25 attended college without receiving a degree. This analysis controls for people who are no longer attending school so that those actively taking courses but not yet having a degree are not shown. (This mainly impacts the 18-24 age range). Since for this analysis, I am top coding educational attainment based on highest degree received, this entire group is considered at the same level as those to only graduate high school without attending any college courses. It is unclear how the percentage of adults with only some college looks when compared over time, but it is notable that this percentage seems to be trending upwards. As the population of the United States is increasing this corresponds with a larger number of people who attended at least one college course but do not have a degree. Perhaps college completion is a more important statistic than mere college enrollment.
...
Looking at the above we see that our pattern of increased education leading to increased income holds true across all races, although it is worth noting that the income for each education level is not the same across each race. Higher education attainment clearly corresponds to higher incomes for individuals, but those gains are not distributed evenly across the board. For example, White people with a bachelors degree on average make as much as peole in the Other and African American categories with a PhD. 

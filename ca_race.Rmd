...
Race.data <- filter.data[, .(year, PUMA10, age, citizen, PINCP, PWGTP,
                             ...
                             race, Hispanic, sex, Occ, Soc)]
Race.data[, year := as.Date(year, "%Y-%m-%d")]
Race.data[, nage := round_any(AGEP, 7)]
Race.data[, white := ifelse(race == "White", "White", "Non-white")]
```

Before we dive into the data let's take a look at the mean incomes for each race over the period in constant 2014 USD and take a look at the unemployment average unemployment rate for each race over the period. For our average income data we will simply look at respondents who are either self-employed or employed.
...
control.data %>% 
    ...
    labs(caption = "Over time all but Asians in Education see \ndecreasing income relative to Whites",
         x = "Age",
...
```{r "Income Growth by Race", fig.height=6}
...
# Remove older lawyers with higher growth rates who make the axes 
    ...
    geom_line() +
    facet_wrap(~ Occ, nrow = 2, ncol = 3) +
    ggtitle(label = "On Average Growth rates of White people were smaller for select Industries",
            subtitle = "Average Growth Rate for years 2010-2014, age bins 21-63") +
    labs(caption = "At least 200 weighted observations per point: Simeon Markind",
         x = "Age",
...
# Remove older lawyers with higher growth rates who make the axes 
    ...
    geom_line() +
    facet_wrap(~ Occ, nrow = 2, ncol = 3) +
    ggtitle(label = "Average Growth rates for men and women did not differ dramatically for select industries",
...
ratio.data[, .(growth = round(mean(avg_growth, na.rm = T),3)), .(race, Occ)] %>% 
    ...
    new_kable(digits = 3, col.names = c("Occupation", "African American", "Asian", "Other", "White"))
```
...
Let's start with a map looking at average income for each race in California in 2014.

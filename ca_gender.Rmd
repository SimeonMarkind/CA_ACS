...
So at first glance we see that women do seem to substantially make less than men, there were no rows in which women made more than men did, and we see a clear trend seeming to indicate that as people age the gender pay gap increases. However, this could be due to many factors: for example it could be that women work less hours than men, that women work in fields or in jobs that pay everyone less, that women leave the workforce in a much greater percentage than men, or other causes. 

```{r "Density of Income by Gender"}
bin_inc <- gender.data[empStat != "Unemployed", .(inc = weighted.mean(PINCP, PWGTP)), .(sex)]

gender.data[empStat != "Unemployed", .(PINCP, PWGTP, sex)] %>% 
...
Let's take a look at some of these causes. First we will look at hours worked per week using the WKHP variable.

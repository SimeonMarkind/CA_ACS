...
gender.data <- filter.data[PINCP > 0, .(year, PUMA10, age, citizen, PINCP, PWGTP,
...
gender.data[, nage := round_any(AGEP, 7)]
gender.data[, year := as.Date(year, format = "%Y-%m-%d")]
...
For our data we have a sample of `r format(nrow(gender.data), big.mark=",")` observations with roughly 130,000 raw observation for each gender for each year, (roughly 2.7 million weighted observations per gender per year). Below is a table of mean salary by gender and age.


```{r "mean salary", fig.align="center"}
salary.mean <- gender.data[, .(salary = weighted.mean(PINCP, PWGTP)),
                .(sex, age)][order(age, sex)]

salary.mean <- dcast(salary.mean, age ~ ..., value.var = "salary")

new_kable(salary.mean, digits = 2, col.names = c("Age", "Female", "Male"), caption = "Mean Salary by Gender and Age Range")
```

So at first glance we see that women do seem to substantially make less than men, there were no rows in which women made more than men did, and we see a clear trend seeming to indicate that as people age the gender pay gap increases. However, this could be due to many factors: for example it could be that women work less hours than men, that women work in fields or in jobs that pay everyone less, that women leave the workforce in a much greater percentage than men, or other causes. Let's take a look at some of these causes. First we will look at hours worked per week using the WKHP variable.
...
casted.data[Occ %in% c("Admin Support", "Legal", "Education", 
    ...
    scale_x_continuous("Age", breaks = seq(20, 80, 10)) +
...
mobility.data <- filter.data[PINCP > 0, .(county, MIGSP05, MIGSP12, Occ, PUMA00, PUMA10,
...
Ratio of Male to Female income earners does not seem to be particularly correlated with the ratio of income. Sacramento county, where women make up over 50% of income earners has an income only slightly lower than Kings county, where women make up only 43% of income earners.
...
```{r "CA Gender Map"}
map_data <- gender.data[year == as.Date("2014-01-01"),
...
ggplot(aes(x = long, y = lat, group = group, fill = percent)) +
    ...
    ggtitle("Men as Percent of Population Above Age 18",
            subtitle = "2014 PUMAS") +
    scale_fill_gradient(NULL, low = "Green", high = "Red") + 
    labs(x = NULL, y = NULL, fill = NULL, caption = "Simeon Markind")
```
...
bachelor_data <- gender.data[nage %in% c(18:35) &
